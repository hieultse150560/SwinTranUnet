...
def findFrame(ts_target, ts_set): # Find middle frame
...
def draw_channel(color, heatmap, keypoint_num, size):
    ...
    heatmap_viz = heatmap[:, :, :, None] * color[:keypoint_num, None, None, :] # None là thêm 1 chiều với dimension bằng 1
    heatmap_viz = np.sum(heatmap_viz, 0)
    heatmap_viz = cv2.resize(heatmap_viz,(size[0],size[1]))
    return heatmap_viz

# Vẽ keypoint 2D trên nền trắng
